...
from .rope_neuron import rope_params_neuron, rope_apply_neuron
...
# @autocast removed for Neuron
def _rope_params_original(max_seq_len, dim, theta=10000):
    assert dim % 2 == 0
    freqs = torch.outer(
...
class WanRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        return self._norm(x.to(torch.bfloat16)).type_as(x) * self.weight
...
class WanLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        ...
        Args:
            x(Tensor): Shape [B, L, C]
        """
        return super().forward(x.to(torch.bfloat16)).type_as(x)
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            freqs(Tensor): Rope freqs, shape [1024, C / num_heads / 2]
        """
        if True:  # autocast removed for Neuron
        ...
        def cross_attn_ffn(x, context, context_lens, e):
            ...
            y = self.ffn(
                self.norm2(x).to(torch.bfloat16) * (1 + e[4].squeeze(2))
                + e[3].squeeze(2)
            )
            if True:  # autocast removed for Neuron
                x = x + y * e[5].squeeze(2)
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.freqs = torch.cat(
            [
                rope_params_neuron(1024, d - 4 * (d // 6)),
                rope_params_neuron(1024, 2 * (d // 6)),
                rope_params_neuron(1024, 2 * (d // 6)),
            ],
            dim=1,
        )
